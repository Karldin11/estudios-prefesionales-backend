...
const getAllPosts = async (req, res, next) => {
  let posts;
  try {
    posts = await Publicaciones.find({});
  } catch (err) {
...
const getPostsBySection = async (req, res, next) => {
  const seccion = req.params.seccion;

  let postsSeccion;
  try {
    postsSeccion = await Publicaciones.find({ seccion });
  } catch (err) {
    const error = new HttpError(
  ...
  res.json({
    posts: postsSeccion.map((postsSeccion) =>
      postsSeccion.toObject({ getters: true })
    ),
  });
};
...
const getPostById = async (req, res, next) => {
  ...
  let post;
  try {
    post = await Publicaciones.findById(postId);
  } catch (err) {
    const error = new HttpError(
...
const createPost = async (req, res, next) => {
  console.log("entro a crear post");
  ...
  const { titulo, seccion, resumen, informacion } = req.body;

  const createdPost = new Publicaciones({
    titulo,
    seccion,
    resumen,
    informacion,
...
const updatePost = async (req, res, next) => {
  ...
  const { titulo, seccion, resumen, informacion, imagen } = req.body;
  const postId = req.params.pid;
