...
const Coordinacion = require("../models/coordinaciones");

const getAllPosts = async (req, res, next) => {
  let posts;
  try {
    posts = await Coordinacion.find({});
  } catch (err) {
...
const getPostsByDepartment = async (req, res, next) => {
  ...
  try {
    postsDepartamentos = await Coordinacion.find({ departamento });
  } catch (err) {
    const error = new HttpError(
...
const getPostById = async (req, res, next) => {
  ...
  let post;
  try {
    post = await Coordinacion.findById(postId);
  } catch (err) {
    const error = new HttpError(
...
const createPost = async (req, res, next) => {
  ...
  const createdPost = new Coordinacion({
    carrera,
    departamento,
    datosContacto,
